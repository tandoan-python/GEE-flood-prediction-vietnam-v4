...
def get_flood_data(s1_baseline, s1_during, slope, features_to_add):
    ...
    """
    # Tinh toan su thay doi (Difference)
    # VH truoc lu (kho) thuong cao hon VH trong lu (uot)
    # Vi du: (-20) - (-25) = 5
    diff = s1_baseline.subtract(s1_during).rename('s1_diff')
    
    # Nguong phat hien thay doi. 5dB la mot nguong rat manh, 
    # chung to su thay doi tu dat sang nuoc.
    flood_map = diff.gt(5)
...
def get_non_flood_data(s1_baseline, s1_during, slope, features_to_add):
    ...
    # va cung o vung trung (de dam bao so sanh tuong dong)
...
def main():
    ...
    for event in FLOOD_EVENTS:
        ...
        time.sleep(60) # Nghi 1 phut giua cac su kien
